...
const Modal = ({ setShowModal }: any) => {
  ...
  const login = () => {
    signInWithPopup(auth, provider)
      .then((data: any) => {
        const { user } = data;
        console.log("🚀 ~ file: Modal.tsx:24 ~ .then ~ data:", data)
  ...
  const loginCredentials = async () => {
    ...
    try {
      ...
      console.log("🚀 ~ file: Modal.tsx:66 ~ loginCredentials ~ data:", data)
      setUser(data.user);
      localStorage.setItem('user', JSON.stringify(data.user));
      setShowModal(false);
  ...
  const registerUser = async () => {
    ...
    try {
      ...
      console.log('🚀 ~ file: Modal.tsx:90 ~ registerUser ~ signIn:', signIn);
      if (signIn.user.uid) {
        setShowModal(false);
      }
      // Redirect or perform other actions upon successful login
